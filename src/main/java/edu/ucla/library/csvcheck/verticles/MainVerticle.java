...
package edu.ucla.library.csvcheck.verticles;
...
import edu.ucla.library.csvcheck.Config;
import edu.ucla.library.csvcheck.MessageCodes;
import edu.ucla.library.csvcheck.Op;
import edu.ucla.library.csvcheck.handlers.StatusHandler;
...
public class MainVerticle extends AbstractVerticle {
     ...
     */
    private static final String API_SPEC = "src/main/resources/csv-check.yaml";
    ...
    private void configureServer(final JsonObject aConfig, final Promise<Void> aPromise) {
        ...
        RouterBuilder.create(vertx, getRouterSpec()).onFailure(aPromise::fail).onSuccess(routeBuilder -> {
            ...
            myServer.listen().onFailure(aPromise::fail).onSuccess(result -> {
                LOGGER.info(MessageCodes.CSV_001, port);
                aPromise.complete();
            });
        });
    }
